...
export async function odataConnector(url: URL, options?: { baseUrl?: string, token?: string, isPortal?: boolean }): Promise<ApiConnector> {
	...
	const navPropCache: Record<string, NavPropertyData[]> = {};
	...
	async function create(entityName: string, data: EntityData): Promise<Guid> {
		await preprocessDataForUpdate(entityName, data);
	...
	async function preprocessDataForUpdate(entityName: string, data: EntityData): Promise<void> {
	...
	async function getNavPropName(entityName: string, fieldName: string, targetEntity: string): Promise<string | undefined> {
		...
		if (key in navPropCache)
			return navPropCache[key]?.find(v => v.ReferencedEntity.toLowerCase() === targetEntity.toLowerCase() && v.ReferencingAttribute.toLowerCase() === fieldName.toLowerCase())?.ReferencingEntityNavigationPropertyName;

		// not found, so we need to fetch it from the API
		const url = `EntityDefinitions(LogicalName='${entityName.toLowerCase()}')/ManyToOneRelationships?$select=ReferencedEntity,ReferencingAttribute,ReferencingEntityNavigationPropertyName&$filter=ReferencingAttribute eq '${fieldName.toLowerCase()}'`
		const result = await get<{ value: NavPropertyData[] }>(url)

		// store for later use in cache
		navPropCache[key] = result.value;
		console.debug(result.value.length);
